...
| `resolveAliasPath(aliasPath)`         | Converts a path string containing a Vite alias (e.g., `@Core/path`) into a server-relative path (e.g., `/src/core/path`) necessary for dynamic runtime imports. **Note:** The alias mapping must correspond to the configuration in `vite.config.mts`. | `aliasPath`: `string` - The path string possibly containing an alias.                                                                                                       | `string` - The resolved path string.                                  |
