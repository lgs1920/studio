...
| 5 Terrain correction persistence | Implemented | Terrain collision handling now persists as terrain redirect annotations on the compiled replay path | Broader path-shape regression coverage is still useful | Keep the persisted path annotations and extend coverage when new redirect modes appear |
