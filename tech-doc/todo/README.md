# Implementation TODO documentation

This directory contains proposed, pending-validation, explicitly TODO, or future implementation specifications.

- [Cloud sync](CLOUD-SYNC-TODO.md)
- [Weekly count API](COUNT_API_SPEC.md)
- [Weekly count API — backend issue](COUNT_API_SERVER_ISSUE.md)
- [Weekly count API — studio issue](COUNT_API_STUDIO_ISSUE.md)
- [HQ 4K video export](HQ_4K_VIDEO_EXPORT_SPEC.md)
- [Journey import formats](JOURNEY_IMPORT_FORMATS_SPEC.md)
- [Translation notes](Translate.md)
- [Drone camera 3D path editor](src/core/ui/camera/DRONE_CAMERA_3D_PATH_EDITOR_SPEC.md)
- [Replay track timeline editor](src/core/ui/replay/REPLAY_TRACK_TIMELINE_EDITOR_EVOLUTION.md)
- [Cesium Ion layers](src/components/cesium/ion-layers-spec.md)
- [Tracks editor](src/components/TracksEditor/spec.md)
- [Profile TODO](src/components/Profile/TODO.md)
- [Main UI TODO](src/components/MainUI/TODO.md)
- [Brand and season swatch reactivity](BRAND_SEASON_SWATCH_REACTIVITY_SPEC.md)
- [Arrow widget](ARROW_WIDGET_SPEC.md)
- [Video widget](VIDEO_WIDGET_SPEC.md)
- [Replay video architecture](src/core/ui/replay/REPLAY_VIDEO_ARCHITECTURE.md)
- [Clip altitude data alignment](src/core/ui/replay/CLIP_ALTITUDE_DATA_ALIGNMENT_SPEC.md)
