...
import {
    FLAG_START, FLAG_STOP, JUST_ICON, POI_FLAG, POI_STD,
...
export class Journey extends MapElement {
    ...
    getPOIsFromGeoJson = () => {
        if (this.geoJson.type === FEATURE_COLLECTION) {
            ...
            this.geoJson.features.forEach((feature, index) => {
                ...
                switch (geometry.type) {
                    ...
                    case FEATURE_MULTILINE_STRING: {
                        ...
                        const startFlag = __.ui.poiManager.add({...common, ...startParameters})
                        ...
                        const stopFlag = __.ui.poiManager.add({...common, ...stopParameters})
                        this.tracks.get(parentSlug).flags.stop = stop
            ...
            //     __.ui.poiManager.list.forEach((poi, index) => {
            //         const track = poi.slug.split('#').slice(1,-1).join('#')
            //         this.tracks.get(track).flags[(poi.id.endsWith(FLAG_START)) ? 'start' : 'stop'].visible = (index === 0 || index === (flags.length - 1))
            //     })
            // }
        }
    }
