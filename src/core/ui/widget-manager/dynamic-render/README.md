...
| `isMaxReached(groupKey, widgetKey)`   | Checks if the maximum allowed number of instances for a specific widget has been reached, based on the `lgs.stores.ui.widget.list` store.                                                                                                             | `groupKey`: `string` - Group ID. `widgetKey`: `string` - Base widget ID (without instance suffix).                                                                          | `boolean` - `true` if the maximum limit is reached.                   |
