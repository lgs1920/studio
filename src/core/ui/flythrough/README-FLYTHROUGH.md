...
For `Behind` and `Ahead`, the nominal heading also includes `camera.headingOffset` in degrees, clamped between `-90` and `90`. The drawer slider is intentionally reversed: moving it to the left applies a negative offset in the camera model, and moving it to the right applies a positive one. That offset is applied before the existing heading hysteresis and visibility checks, so it behaves like a simple angular bias on top of the current trace-facing view.
