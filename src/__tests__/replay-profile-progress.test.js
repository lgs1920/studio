 ...
 *
 * File: replay-profile-progress.test.js
 *
 * Author : LGS1920 Team
...
import {
    ...
    buildJourneyReplayCompletedProfileSource,
    createJourneyReplayProfileDatasetLookup,
...
const dimensions = extendJourneyReplayProfileDimensions(['Distance', 'Elevation', 'Time', 'point'])

const row = (distanceFromStart, elevation = 100) => appendJourneyReplayProfileMetadata([
...
describe('replay profile progress', () => {
    it('cuts a completed profile source by cumulative distance and appends the interpolated sample', () => {
        ...
        const lookup = createJourneyReplayProfileDatasetLookup(dataset, dimensions)
        const source = buildJourneyReplayCompletedProfileSource({
            dataset,
            lookup,
            dimensions,
            sample: {
    ...
    it('maps samples and profile rows without losing replay metadata', () => {
        const sample = {
        ...
        const profileRow = replayProfileRowFromSample(sample, {dimensions})
        const sampler = {atDistance: vi.fn(distance => ({...sample, distanceFromStart: distance}))}
        const restored = replaySampleFromProfileRow(profileRow, dimensions, sampler)
