 ...
 *
 * File: flythrough-stats-widget-visibility.test.js
 *
 * Author : LGS1920 Team
...
describe('flythrough stats widget visibility', () => {
    beforeEach(() => {
        globalThis.lgs = {
            theJourney: {
                slug: 'journey-a',
    ...
    it('shows the dynamic stats widget while the flythrough is active and not near the end', () => {
        expect(shouldShowDynamicStatsWidget(globalThis.lgs.stores.flythrough)).toBe(true)
    })

    it('hides the dynamic stats widget once the end threshold is reached without stop clips', () => {
        globalThis.lgs.stores.flythrough.progress = 0.9
        expect(shouldShowDynamicStatsWidget(globalThis.lgs.stores.flythrough)).toBe(false)
    })

    it('shows the journey stats widget only near the end of the flythrough', () => {
        globalThis.lgs.stores.flythrough.progress = 0.9
        expect(shouldShowJourneyStatsWidget(globalThis.lgs.stores.flythrough)).toBe(true)

        globalThis.lgs.stores.flythrough.progress = 0.2
        expect(shouldShowJourneyStatsWidget(globalThis.lgs.stores.flythrough)).toBe(false)
    })

    it('builds live metrics from the flythrough sample', () => {
