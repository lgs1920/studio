...
- Before drafting a changelog, collect every verified issue closed in the release scope from each owning repository. Use the full dataset to identify the major delivered work, prioritizing the areas with the greatest concentration of completed issues and then their user-visible impact.
- The `New Features and Improvements` section describes those major delivered outcomes, ordered by the amount and impact of the completed work. It must not replace the detailed list of closed issues or promote minor fixes above larger delivered areas.
