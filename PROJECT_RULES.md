...
- The `Closed Issues` section lists at most the 10 latest individual issues for each application, keeps each issue under its owning application/repository, and sorts those displayed issues by descending fix/closure date. Do not merge or synthesize multiple issues into one item.
