...
- After the user validates the proposed changelog, set the draft date and every applicable current-release date field to today's execution date. Rename the current draft to today's `YYYYMMDD-<version>.md` filename when necessary. Never change dates in published historical changelogs.
- After that validation, when the target changelog does not exist, create it automatically using today's date and the naming/header convention of the closest existing changelog for the same release line. Never overwrite an existing changelog merely to normalize its historical formatting.
...
- Before drafting a changelog, collect every verified issue closed in the release scope from each owning repository. Compare the completed work with the previous release to identify the product-level transformations it delivers.
- The `New Features and Improvements` section describes those product-level outcomes, ordered by user impact. Consolidate related implementation work under the outcome it enables: for example, Timeline work belongs under a broader video-creation overhaul when that is the delivered transformation. It must not replace the detailed list of closed issues.
- Present the proposed release scope, major-improvement text, and complete closed-issue list to the user before creating, renaming, or editing a changelog. Write the changelog only after the user explicitly validates that proposal.
- Keep `New Features and Improvements` concise and outcome-focused. Use a repository-specific GitHub compare link when readers need the implementation detail behind a broad delivered transformation. Use only verified previous and current version tags, commit references, or release references; for Site, use verified commit or release references.
- The `Closed Issues` section lists at most the 10 latest individual verified issues for each application, keeps each issue under its owning application/repository, and sorts them by descending fix/closure date. Do not merge or synthesize multiple issues into one item.
- When an application has more than 10 closed issues for the release, display only its 10 latest issues and add the exact prefix `Full list: ` followed by a repository-specific GitHub issues search link listing the issues closed since the previous release version. Use the verified previous-release boundary and repository ownership filter.
- For `site` and `backend`, include every issue closed on or after the date of the latest previous Studio release in the release dataset, even when the issue has no matching Project `Target release` or milestone. Use the date in the latest previous Studio changelog filename as the boundary, keep each issue under its owning repository, and apply the display cap when required.
