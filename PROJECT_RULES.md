...
- The `Closed Issues` section lists every individual verified issue closed for the release, keeps each issue under its owning application/repository, and sorts them by descending fix/closure date. Do not merge or synthesize multiple issues into one item.
