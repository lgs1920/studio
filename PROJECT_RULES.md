...
- For each application with more than 5 closed issues, keep the complete issue list and add a repository-specific GitHub issues search link showing all closed issues included in the release. Use the same verified release boundary and ownership filters used to build the list. When an application has 5 or fewer closed issues, list the issues without requiring this additional link.
